...
export default function EventModal() {
  ...
  const performSave = async (updateFutureSerie) => {
    ...
    const calObj = calendars.find(
      (c) => c.id === selectedCalendar || c.docId === selectedCalendar,
    );
    const idForDB =
      calObj && calObj.isShared ? calObj.originalId : selectedCalendar;
    ...
    try {
      ...
      if (
        ...
        selectedEvent.day !== localEvent.day
      ) {
        const duplicatedEventLocal = {
          ...localEvent,
          id: Date.now().toString(),
          checked: false,
          postponable: true,
        };
        const duplicatedEventDB = {
          ...dbEvent,
          id: duplicatedEventLocal.id,
          checked: false,
          postponable: true,
        };

        await setDoc(
          doc(db, `users/${targetUserId}/events`, duplicatedEventDB.id),
          duplicatedEventDB,
        );
        dispatchCalEvent({ type: "push", payload: duplicatedEventLocal });

        const updatedPostponableEventLocal = {
          ...selectedEvent,
          checked: true,
        };
        ...
        const calObjOriginal = calendars.find(
          (c) =>
            c.id === selectedEvent.calendarId ||
            c.docId === selectedEvent.calendarId,
        );
        const originalIdForDB =
          calObjOriginal && calObjOriginal.isShared
            ? calObjOriginal.originalId
            : selectedEvent.calendarId;
        const updatedPostponableEventDB = {
          ...updatedPostponableEventLocal,
          calendarId: originalIdForDB,
        };

        await setDoc(
          doc(db, `users/${originalOwnerId}/events`, selectedEvent.id),
          updatedPostponableEventDB,
        );
        dispatchCalEvent({
          type: "update",
          payload: updatedPostponableEventLocal,
        });
      }
      // 2. AGGIORNAMENTO DI UNA SERIE ("Modifica Futuri")
        ...
        await batch.commit();
        updatedEventsLocal.forEach((ev) =>
          dispatchCalEvent({ type: "update", payload: ev }),
        );
      }
      // 3. SALVATAGGIO SINGOLO O NUOVO EVENTO
      else {
        await setDoc(
          doc(db, `users/${targetUserId}/events`, localEvent.id),
          dbEvent,
        );
        if (selectedEvent) {
  ...
  async function createRepeatedEvents(
    localEvent,
    dbEvent,
    repeatOpts,
    startDateArg = new Date(localEvent.day),
  ) {
    ...
    if (endOption === "never") {
      ...
      calculatedEndDate = dayjs(localEvent.day)
    ...
    if (frequency === "week") {
      ...
      for (
        ...
        for (let i = 0; i < daysOfWeek.length; i++) {
          ...
          if (
            weekIndex >= weeksMatrix.length ||
            dayIndex >= weeksMatrix[weekIndex].length ||
            !day
          )
            continue;
          ...
          if (
            day ===
              dayjs(localEvent.day).locale("en").format("ddd, D MMM, YYYY") ||
            eventDate < dayjs(localEvent.day).toDate()
          )
            continue;

          const repeatedEventLocal = {
            ...localEvent,
            day: eventDate.getTime(),
            id: `${localEvent.id}-${eventCount}`,
          };
          const repeatedEventDB = {
            ...dbEvent,
            day: eventDate.getTime(),
            id: `${localEvent.id}-${eventCount}`,
          };
          ...
          batch.set(
            doc(db, `users/${targetUserId}/events`, repeatedEventDB.id),
            repeatedEventDB,
          );
      ...
      while (
        ...
        const repeatedEventLocal = {
          ...localEvent,
          day: eventDate.getTime(),
          id: `${localEvent.id}-${currentYear}-${currentMonth}`,
        };
        const repeatedEventDB = {
          ...dbEvent,
          day: eventDate.getTime(),
  ...
  const handleDuplicateSave = async () => {
    ...
    const calObj = calendars.find(
      (c) =>
        c.id === duplicateData.calendarId ||
        c.docId === duplicateData.calendarId,
    );
    const idForDB =
      calObj && calObj.isShared ? calObj.originalId : duplicateData.calendarId;
    ...
    const newEventLocal = {
      ...
      checked: false,
      repeat: repeatId,
      repeatType: duplicateRepeatOptions
        ? getRepeatTypeString(duplicateRepeatOptions)
        : null,
    ...
    <div className="h-screen w-full fixed left-0 top-0 flex justify-center items-center bg-black bg-opacity-50 z-51 dark:bg-zinc-800 dark:bg-opacity-75">
                ...
                <div className="flex flex-row">
                  ...
                  <div className="grid grid-cols-6 gap-x-6 gap-y-4 ml-6 hidden md:!grid">
                    {labels
                      .filter(
                        (label) =>
                          label.calendarId === duplicateData.calendarId,
                      )
      ...
      >
          ...
          <div className="flex flex-col gap-y-4">
            ...
            <div className="flex flex-col md:flex-row md:items-center mb-8 mt-2 md:mt-0 gap-y-4">
              ...
              <div className="flex flex-col gap-4">
                {/* Calendari - Desktop */}
                <div className="ml-16 hidden md:grid grid-cols-4 gap-x-10 gap-y-5 items-center">
                  {sortedCalendar.map((cal, i) => (
                    <div
                      key={cal.id || i}
                      onClick={() => setSelectedCalendar(cal.id)}
                      className={`flex items-center justify-center cursor-pointer rounded w-40 h-8 border transition-all ${
                        selectedCalendar === cal.id
                          ? "bg-blue-700 border-blue-900 border-2"
                          : "bg-blue-300 border-transparent hover:bg-blue-400"
                      }`}
                    >
                      <span className="text-black font-bold text-sm">
                        {cal.name}
                ...
                <div className="ml-16 md:hidden">
                  <select
                    ...
                    className="border rounded-2xl p-2 dark:bg-zinc-700 dark:text-white px-4 outline-none"
                  >
                    <option value="" disabled>
                      Scegli il calendario
                    </option>
                    {sortedCalendar.map((cal, i) => (
                      <option key={cal.id || i} value={cal.id}>
                        {cal.name}
                      </option>
                    ))}
                  </select>
            ...
            <div className="flex flex-col md:flex-row md:items-start mt-6 gap-y-4">
              ...
              {/* Labels - Desktop */}
              <div className="ml-16 hidden md:grid grid-cols-6 gap-x-10 gap-y-5">
                {labels
                  ...
                  .map((lbl, i) => (
                    <div
                      ...
                      className="flex items-center justify-center cursor-pointer rounded w-40 h-8 transition-all"
                      style={{
                        backgroundColor:
              ...
              <div className="ml-16 md:hidden">
                ...
                >
                  ...
                  {labels
                    ...
                    .sort((a, b) => a.code - b.code)
                    .map((lbl, i) => (
                      <option key={lbl.id || i} value={lbl.name}>
                        {lbl.name}
                      </option>
                    ))}
                </select>
        ...
        <footer className="flex justify-end items-center p-4 mt-5 rounded-b-4xl border-t border-gray-200 dark:border-zinc-800">
          {showLabelWarning && (
            <p className="text-red-500 mr-6">{t("select_a_label")}</p>
          )}
