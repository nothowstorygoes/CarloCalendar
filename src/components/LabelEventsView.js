...
export default function LabelEventsModal({ label, setShowLabelEventsModal }) {
  ...
  return (
    <div className="fixed inset-0 flex justify-center items-center z-40 pointer-events-none">
      <div
        ref={modalRef}
        className="bg-white w-[calc(100%-16rem)] h-[calc(100%-4rem)] max-w-none max-h-none overflow-hidden relative ml-64 mt-16 pointer-events-auto z-50"
      >
        <hr></hr>
        <div className="p-4 overflow-auto relative">
          <div className="flex items-center justify-between mb-6 w-full">
            <h2 className="text-lg font-bold text-left mb-6 ml-6 mt-5">
              Events for{" "}
              <span style={{ color: label.color }}>{label.name}</span>
            </h2>
            <button
              onClick={() => setShowLabelEventsModal(false)}
              className="bg-gray-200 hover:bg-gray-300 text-gray-600 rounded-full p-2 w-10 h-10 flex items-center justify-center mr-10"
            >
