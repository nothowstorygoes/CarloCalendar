...
export default function Profile() {
  ...
  const { user, viewMode, setViewMode, savedEvents } =
  ...
  const handleDownloadAllPDF = async () => {
    ...
    const filteredEvents = savedEvents
      .filter((event) => {
        ...
        return (
          eventDate.isAfter(start) && eventDate.isBefore(end) && !event.checked
        );
      })
      .map((event) => {
        ...
        return {
          ...event,
          labelCode: label ? label.code : 0,
          labelColor: label ? label.color : "#000000",
        };
      })
      .sort((a, b) => {
    ...
    doc.setFontSize(12);
    doc.text(
      `Eventi dal ${format(start.toDate(), "dd/MM/yyyy")} al ${format(
        end.toDate(),
        "dd/MM/yyyy"
      )}`,
      105,
  ...
  if (!userInfo) {
    return (
      <div className="h-[48.5rem] md:mt-0 -mt-4 w-screen md:h-[calc(100%-4rem)] md:w-[calc(100%-1.5rem)] left-0 top-0 flex justify-center items-center bg-white dark:bg-zinc-900 md:dark:bg-zinc-950 rounded-3xl">
  ...
  return (
    <div className="h-[47.5rem] w-screen md:h-[calc(100%-4rem)] md:w-[calc(100%-1.5rem)] left-0 top-0 flex justify-center md:items-center bg-white dark:bg-zinc-900 md:dark:bg-zinc-950 rounded-3xl">
      <div className="bg-white dark:bg-zinc-900 md:dark:bg-zinc-950 w-screen h-[40rem] md:w-[calc(100%-16rem)] md:h-[calc(100%-4rem)] max-w-none max-h-none overflow-hidden relative md:mt-8">
        <div className="p-4 relative w-full">
          <div className="flex justify-center flex-col md:flex-row md:justify-between overflow-x-hidden custom-scrollbar">
            <div className="flex flex-col items-center md:items-start mt-12 md:mt-0">
              <p className="text-2xl font-bold text-gray-600 dark:text-zinc-50 mb-5 md:mb-20">
                Ciao, {userInfo.name}
              </p>
              <img
                src={`data:image/png;base64,${userInfo.profilePicture}`}
                alt="Profile"
                className="w-40 h-40 rounded-full mb-4 object-cover md:hidden cursor-pointer"
                onClick={() =>
                  document.getElementById("mobileProfilePictureInput").click()
                }
              />
              <input
                type="file"
                id="mobileProfilePictureInput"
                accept="image/*"
                onChange={handleProfilePictureUpload}
                className="hidden"
              />
              <p className="text-sm text-gray-500 dark:text-zinc-50 mt-12 md:mt-0">
                Email: <b>{userInfo.email}</b>
              </p>
              <div className="flex md:justify-between items-center mt-6 md:mt-12 hidden md:!flex">
                <p className="dark:text-white md:mr-5 text-sm">
                  Scegli un'immagine come avatar:
                </p>
                <input
                  type="file"
                  accept="image/*"
              ...
              <div className="flex flex-col justify-center md:flex-row md:justify-between items-center mt-12">
                <button
                  className="bg-blue-500 hover:bg-blue-600 text-white p-2 rounded-4xl w-40 mb-10 md:mb-0"
                ...
                <button
                  ...
                  className="bg-red-500 md:ml-14 hover:bg-red-600 text-white p-2 rounded-4xl w-40"
                >
                  {t("sign_out")}
                </button>
              </div>
            </div>
            <div className="flex flex-col mt-12 hidden md:!flex">
              ...
              />
              <label className="text-gray-600 dark:text-zinc-50 mt-4">
                Fine
              </label>
              <input
                type="date"
